...
HAIR_TYPES = [
    ...
    "Окрашенные",  # ПРАВКА #2: Было "Окрашенные все остальные"
    "Натуральные"
]
...
def get_hair_colors(hair_type: str):
    ...
    if hair_type == "Окрашенные блондинки":
        return ["Блондинка"]
    elif hair_type == "Окрашенные":  # ПРАВКА #2: Было "Окрашенные все остальные"
...
PHOTO_MAPPING = {
    ...
    "волосы": {
        ...
        "Окрашенные": ["colored_conditioner", "colored_mask"],  # ПРАВКА #2: Было "Окрашенные все остальные"
        "Натуральные": [],
...
HAIR_DATA = {
    ...
    "problems": {
        "Ломкость": {
            ...
            "products": ["Биолипидный спрей", "Флюид для волос", "Масло ELIXIR", "Протеиновый крем", "Укрепляющий спрей для волос"]  # ПРАВКА #4: Добавлено
        },
        "Выпадение": {
        ...
        "Пушистость": {
            ...
            "products": ["Флюид для волос", "Протеиновый крем", "Масло ELIXIR", "Молочко для волос"]  # ПРАВКА #6: Добавлено
        },
        "Тонкие и лишенные объема": {
            "title": "⚡ <b>Для решения проблемы тонких волос:</b>",
            "products": ["Шампунь для тонких волос", "Кондиционер для тонких волос", "Маска укрепление и сила", "Укрепляющий спрей для волос"]  # ПРАВКА #7: Добавлено
        },
        "Очень поврежденные": {
...
def get_hair_recommendations_html(hair_type: str, problems: list, scalp_type: str, 
    ...
    elif hair_type == "Окрашенные":  # ПРАВКА #2
    ...
    # ПРАВКА #8: Оттеночные маски только для окрашенных, не для натуральных
    if hair_type == "Окрашенные":  # Только для окрашенных волос
...
DELIVERY_INFO = (
    "<b>🚚 Доставка:</b>\n"
    "Вы можете заказать доставку выбранных продуктов, перекинув свой заказ в личку @SVOY_AVCOSMETIC"  # ПРАВКА #5
)
